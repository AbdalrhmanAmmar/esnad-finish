...
export const updateVisit = async (
  medicalRepId: string,
  visitId: string,
  visitData: Partial<CreateVisitRequest>
): Promise<ApiResponse<VisitResponse>> => {
  try {
...
export const getDetailedVisits = async (
  medicalRepId: string,
  params?: DetailedVisitsParams
): Promise<ApiResponse<DetailedVisitsResponse>> => {
  try {
    ...
    const url = `/visit-forms/medical-rep/${medicalRepId}/detailed-visits${
...
export const exportVisitsToExcel = async (
  medicalRepId: string,
  params?: DetailedVisitsParams
): Promise<Blob> => {
  const queryParams = new URLSearchParams();
  ...
  const response = await api.get(
    `/visit-forms/medical-rep/${medicalRepId}/export-excel?${queryParams.toString()}`,
    {
      responseType: "blob",
    }
  );
...
export const getVisitsBySupervisor = async (
  supervisorId: string,
  params?: GetVisitsBySupervisorParams
): Promise<ApiResponse<SupervisorVisitsResponse>> => {
  const queryParams = new URLSearchParams();
  ...
  if (params?.withSupervisor !== undefined)
    queryParams.append("withSupervisor", params.withSupervisor.toString());
  if (params?.sortBy) queryParams.append("sortBy", params.sortBy);
  if (params?.sortOrder) queryParams.append("sortOrder", params.sortOrder);

  const response = await api.get(
    `/visit-forms/supervisor/${supervisorId}/visits?${queryParams.toString()}`
  );
  return response.data;
};
...
export const getVisitAnalytics = async (
  ...
  try {
    ...
    const queryString = queryParams.toString();
    const url = `/visit-forms/medical-rep/${medicalRepId}/analytics${
      queryString ? `?${queryString}` : ""
    }`;
